...
```
[getUserMedia (mic)] ──┐
                       ├── Web Audio API (AudioContext) ── MediaRecorder (opus/webm)
[getDisplayMedia (tab)]┘         │
                         ...
                         [Deepgram WebSocket]
                                 ...
                                 ▼
                         [Redis Pub/Sub] ── fan out to services
```
...
Claude Opus 4.6 prompt caching is the single most important optimization:
...
```bash
# Initialize project
pnpm create next-app@15.5 life-insurance-copilot --typescript --tailwind --app --src-dir

# Core dependencies
pnpm add @anthropic-ai/sdk @deepgram/sdk ws ioredis drizzle-orm pgvector zustand zod pino

# Database
pnpm add postgres  # PostgreSQL driver for Drizzle
pnpm add -D drizzle-kit

# Dev dependencies
pnpm add -D @types/ws typescript tailwindcss postcss autoprefixer
pnpm add -D dotenv pino-pretty
```
